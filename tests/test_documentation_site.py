...
def test_site_validation_and_registry_derived_routes_are_complete() -> None:
    ...
    assert any(item["route"] == "/docs/mrd-specification/" for item in entries)
    assert any(item["route"] == "/docs/repository/" for item in entries)
    assert any(item["route"] == "/specification/mrd-specification/" for item in entries)
...
def test_site_build_has_entry_points_navigation_and_breadcrumbs(tmp_path: Path) -> None:
    ...
    assert (output / "docs/mrd-specification/index.html").is_file()
    assert (output / "docs/repository/index.html").is_file()
    assert (output / "specification/mrd-specification/index.html").is_file()
    ...
    page = (output / "docs/mrd-specification/index.html").read_text(encoding="utf-8")
    assert "breadcrumbs" in page
    assert "rel=\"next\"" in page
    assert "/specification/mrd-specification/001-specification/" in page
    validation_page = (output / "specification/mrd-specification/009-validation-and-enforcement/index.html").read_text(encoding="utf-8")
...
def test_site_detects_broken_source_link_and_orphan(tmp_path: Path) -> None:
    ...
    index = root / "generated/mrd-specification-docs/000-index.md"
    index.write_text(index.read_text(encoding="utf-8").replace("002-classify-and-select.md", "missing.md"), encoding="utf-8")
...
def test_site_manifest_inputs_bind_only_site_published_families(tmp_path: Path) -> None:
    ...
    assert "generated/mrd-specification-docs/000-index.md" in inputs
...
def test_markdown_renderer_preserves_expected_document_formatting() -> None:
    source = ROOT / "generated/mrd-specification/009-validation-and-enforcement.md"
...
def test_site_validates_nested_markdown_and_fragments(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    shutil.copytree(ROOT, root, ignore=shutil.ignore_patterns(".git", ".venv", ".work"))
    nested = root / "generated/mrd-specification-docs/nested/orphan.md"
    ...
    nested.unlink()
    index = root / "generated/mrd-specification-docs/000-index.md"
    index.write_text(index.read_text(encoding="utf-8").replace("002-classify-and-select.md)", "002-classify-and-select.md#missing-anchor)"), encoding="utf-8")
    codes = {item["code"] for item in validate_documentation_site(root)["diagnostics"]}
...
def test_public_site_exposes_publication_status_version_and_authority(tmp_path: Path) -> None:
    output = tmp_path / "site"
    build_documentation_site(ROOT, output)
    page = (output / "specification/mrd-specification/index.html").read_text(encoding="utf-8")
    assert 'class="publication-meta"' in page
    assert "draft" in page
    assert "2.0.0" in page
    assert "urn:uuid:9a543068-203b-5a7a-8cdb-805517ca3487" in page
...
def test_markdown_renderer_emits_self_contained_mermaid_svg() -> None:
    source = ROOT / "generated/mrd-specification/008-lifecycle.md"
    markdown = """```mermaid\nflowchart LR\n  draft[\"draft\"]\n  active[\"active\"]\n  draft --> active\n```\n"""
    rendered = _markdown_html(markdown, source, {}, ROOT, "/kis-mcp-doc")
...
def test_site_mermaid_pages_have_no_external_runtime_dependency(tmp_path: Path) -> None:
    output = tmp_path / "site"
    build_documentation_site(ROOT, output)
    page = (output / "specification/mrd-specification/008-lifecycle/index.html").read_text(encoding="utf-8")
...
def test_markdown_renderer_wraps_long_labels_and_terminates_edges_at_nodes() -> None:
    source = ROOT / "generated/mrd-specification/004-authority-ownership-and-relationships.md"
