...
[Previous: Lifecycle](008-lifecycle.md) | [Index](000-index.md)
...
MRD Specification records MUST validate through the public composed governance MRD profile, which binds the reusable core envelope to MRD Specification content.

<span id="rule-kis-mrd-val-005"></span>
Any structural schema failure in the MRD Specification set MUST short-circuit semantic validation for the entire set, because cross-record semantics are valid only over a structurally valid governance set.
...
A validation result has one of these statuses: `valid`, `invalid`. It reports these check keys: `classification`, `applicability`, `ownership`, `layering`, `dependencies`, `provenance`, `lifecycle`, `schema`. Machine-readable diagnostics on failure are required.
...
This page projects `urn:uuid:361b074c-ddf6-5dcf-9031-dbb4ed306abf` version `2.0.0`. The MRD remains authoritative; this page has no write-back authority.
